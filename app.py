...
def index():
    ...
    if request.method == 'POST':
        try:
            ...
            if option == "mysql":
                conn = {
                    "host": "database-2.cuxnpjy7qt1g.us-east-1.rds.amazonaws.com",
                    "password": "abcd1234",
                    "port": 3306,
                    "user": "admin",
                ...
                server = "ec2-54-90-59-95.compute-1.amazonaws.com"
                port = 27017
                database = "instacart"
                con = pyodbc.connect("DRIVER={Devart ODBC Driver for MongoDB};"+"Server=" +
                                     server + ";Port=" + str(port) + ";Database=" + database, autocommit=True)
                cur = con.cursor()
                tic = time()
                r = cur.execute(query_con)
                if cur.description is not None and r == 0:
                ...
                #     c = 3
                    ...
                    data_row = [list(i) for i in cur.fetchall()]
